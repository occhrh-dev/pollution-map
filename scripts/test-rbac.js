...
function fixture() {
  ...
  user('root','root@example.org','CENTRAL','ADMIN');user('a','a@example.org','A','ADMIN');user('a2','a2@example.org','A','ADMIN');user('b','b@example.org','B','ADMIN');user('viewer','viewer@example.org','A','VIEWER');
  add('CONFIG',['SYSTEM_ADMIN_EMAILS','root@example.org','','SYSTEM']);add('CONFIG',['REGISTRATION_REVIEW_AGENCY_ID','CENTRAL','','SYSTEM']);
  ...
  request('REQ-A','new-a@example.org','A','EXISTING_AGENCY');request('REQ-B','new-b@example.org','B','EXISTING_AGENCY');request('REQ-NEW','founder@example.org','','NEW_AGENCY','New Hospital');
  function sheet(name) {
  ...
  const claims={root:{sub:'root',email:'root@example.org'},a:{sub:'a',email:'a@example.org'},a2:{sub:'a2',email:'a2@example.org'},b:{sub:'b',email:'b@example.org'},viewer:{sub:'viewer',email:'viewer@example.org'},outsider:{sub:'outsider',email:'outsider@example.org'},founder:{sub:'sub-REQ-NEW',email:'founder@example.org'}};
  ...
  function denied(token,action,payload){const result=call(token,action,payload);assert.equal(result.ok,false,action+' unexpectedly succeeded');assert.ok(['ACCESS_DENIED','AUTH_REQUIRED','LAST_ADMIN','ACCOUNT_DISABLED','BAD_REQUEST','NOT_FOUND'].includes(result.code),JSON.stringify(result));return result;}
...
test('System privilege requires verified identity and active central administrator',()=>{const f=fixture();f.denied('outsider','systemSession');f.denied('a','systemSession',{agencyId:'CENTRAL'});f.ok('root','systemSession');f.tables.USERS[1][5]='DISABLED';f.denied('root','systemSession');});
test('Agency administrator sees only its existing-agency requests',()=>{const f=fixture();const data=f.ok('a','listRegistrationRequests',{agencyId:'A'});assert.deepEqual(data.map(x=>x.id),['REQ-A']);f.denied('a','listRegistrationRequests',{agencyId:'B'});f.denied('viewer','listRegistrationRequests',{agencyId:'A'});});
test('Agency administrator cannot approve new agencies even with forged review agency',()=>{const f=fixture();f.denied('a','reviewRegistration',{agencyId:'A',requestId:'REQ-NEW',decision:'APPROVE',role:'ADMIN'});f.denied('a','reviewRegistration',{agencyId:'CENTRAL',requestId:'REQ-NEW',decision:'APPROVE'});f.denied('a','systemReviewRegistration',{requestId:'REQ-NEW',decision:'APPROVE'});assert.equal(f.tables.AGENCIES.length,4);});
test('System administrator creates agency and initial admin only through system endpoint',()=>{const f=fixture();const result=f.ok('root','systemReviewRegistration',{requestId:'REQ-NEW',decision:'APPROVE',role:'VIEWER'});assert.equal(result.role,'ADMIN');assert.equal(f.tables.AGENCIES.length,5);const membership=f.tables.USERS.find(r=>r[3]===result.createdAgencyId);assert.equal(membership[4],'ADMIN');assert.equal(membership[0],'sub-REQ-NEW');f.denied('root','systemReviewRegistration',{requestId:'REQ-A',decision:'APPROVE'});});
...
test('System administrator may manage agency status without suspending central registry',()=>{const f=fixture();f.denied('root','systemSetAgencyStatus',{targetAgencyId:'CENTRAL',status:'SUSPENDED'});f.ok('root','systemSetAgencyStatus',{targetAgencyId:'B',status:'SUSPENDED'});f.denied('b','session',{agencyId:'B'});f.ok('root','systemSetAgencyStatus',{targetAgencyId:'B',status:'ACTIVE'});});
test('Last active agency administrator and central administrator are protected',()=>{const f=fixture();f.denied('b','upsertMember',{agencyId:'B',member:{email:'b@example.org',role:'VIEWER'}});f.denied('b','disableMember',{agencyId:'B',email:'b@example.org'});f.denied('a','upsertMember',{agencyId:'A',member:{email:'root@example.org',role:'ADMIN'}});f.denied('root','systemSetMember',{targetAgencyId:'CENTRAL',member:{email:'root@example.org',role:'VIEWER'}});});
