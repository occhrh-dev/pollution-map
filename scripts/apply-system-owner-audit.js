...
"test('Audit log is system-scoped and dangerous actions require typed confirmation',()=>{const f=fixture();f.denied('a','systemListAuditLog');const logs=f.ok('root','systemListAuditLog',{limit:20});assert.ok(Array.isArray(logs));f.ok('root','systemSetMember',{targetAgencyId:'CENTRAL',member:{email:'backup@example.org',role:'ADMIN',status:'ACTIVE'}});f.denied('root','systemSetMember',{targetAgencyId:'CENTRAL',member:{email:'backup@example.org',role:'ADMIN',status:'DISABLED'}});f.ok('root','systemSetMember',{targetAgencyId:'CENTRAL',member:{email:'backup@example.org',role:'ADMIN',status:'DISABLED'},confirmation:'backup@example.org'});});\n\nconsole.log('RBAC regression suite completed.');",'audit test');
